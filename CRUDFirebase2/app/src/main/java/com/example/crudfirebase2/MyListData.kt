...
class MyListData : AppCompatActivity(), RecyclerViewAdapter.dataListener {
    ...
    private fun GetData() {
        Toast.makeText(
            applicationContext, "Mohon Tunggu Sebentar...",
            Toast.LENGTH_LONG
        ).show()
...
//Memasang Adapter pada RecyclerView
                        ...
                        Toast.makeText(
                            applicationContext, "Data Berhasil Dimuat",
                            Toast.LENGTH_LONG
                        ).show()
                    }
                }
...
RecyclerView* melalui interface. kemudian akan menghapus data berdasarkan primary key dari
data tersebut Jika berhasil, maka akan memunculkan Toast */
        val getUserID: String = auth?.getCurrentUser()?.getUid().toString()
        val getReference = database.getReference()
        if (getReference != null) {
            ...
            Toast.makeText(
                this@MyListData, "Referance Kosong",
                Toast.LENGTH_SHORT
            ).show();
        }
    }
